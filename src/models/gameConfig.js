...
////////GET ALL PLAYER////////////////////////////
    MgetAllConfig: () => {
        return new Promise((resolve, reject) => {
            conn.query('SELECT button.* ,sound.nama_sound FROM button INNER JOIN sound ON button.id_button = sound.id_sound', (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
...
//////// POST / LOGIN  PLAYER ////////////////////////////
    ...
    MloginPlayer: (username) => {
        return new Promise((resolve, reject) => {
            conn.query('SELECT * FROM player WHERE username = ?', username, (err, result) => {
                if (!err) {
                    console.log(result)
                    resolve(result)
                } else {
